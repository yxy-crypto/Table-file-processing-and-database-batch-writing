import pandas as pd
import openpyxl
#将excel表格转化为csv文件，表格结构为：word meaning collocation belong，执行过本程序再直接执行CSV_Progress.py文件即可将csv文件写入。
...
def process_excel_to_csv(excel_path, output_csv_path):
    ...
    # 检查并确保 'word' 和 'meaning' 列非空
    if 'word' not in df.columns or 'meaning' not in df.columns:
        raise ValueError("Excel 文件中缺少必要的 'word' 或 'meaning' 列。")

    # 保留需要的列：'word,meaning,collocation,belong'
    df = df[['word', 'meaning', 'collocation', 'belong']]
...
# 示例用法
excel_path = 'dancishu.xlsx'  # Excel 文件路径
output_csv_path = 'dancishu.csv'  # 输出的 CSV 文件路径
