...
def read_csv_to_mysql(csv_file="word.csv",
                       table_name="study",
                       name_column="word",
                       host="localhost",
                       user="root",
    ...
    try:
        ...
        for chunk in pd.read_csv(csv_file, sep=sep, encoding=encoding, header=None, names=columns, chunksize=chunksize):
            ...
            if not existing_chunk.empty:
                print(f"以下单词由于与数据库中已有数据重复，未被录入：")
                print(existing_chunk[name_column].to_list())

            chunk = chunk[~chunk[name_column].isin(existing_names)]  # 过滤掉已存在的单词

            # 处理belong和incapable列的空值问题
            if 'belong' in chunk.columns:
                chunk['belong'] = chunk['belong'].fillna('daily')  # 为空值赋默认值 daily

            if 'incapable' in chunk.columns:
                chunk['incapable'] = chunk['incapable'].fillna('neutral')  # 为空值赋默认值 neutral
...
# 示例用法
read_csv_to_mysql(csv_file="word.csv", sep=';', encoding='utf-8', header=None)
